...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::table('comments', function (Blueprint $table) {
            ...
            $table->foreign(['director_id'], 'comments_ibfk_4')->references(['id'])->on('users')->onUpdate('no action')->onDelete('no action');
            $table->foreign(['president_id'], 'comments_ibfk_5')->references(['id'])->on('users')->onUpdate('no action')->onDelete('no action');
        });
    }
