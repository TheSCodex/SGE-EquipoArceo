...
@section('contenido')
    <div class=" w-[95%] h-auto mx-auto bg-white rounded-md my-6">
        {{-- Datos Personales --}}
        <div class="my-4">
        ...
        <form method="POST" action="{{ url('alumno') }}" class="space-y-4 ml-4 ">
            <div class="Datos Personales flex gap-5 ">
                ...
                <div class="w-[20%]">
                    ...
                    <input type="number" name="matricula" placeholder="Matricula" value="{{ old('matricula') }}" required
            ...
            <div class="Datos Personales 2 flex gap-5">
                ...
                <div class="w-[13%]">
                    ...
                    @csrf
                    <input type="date" name="Fecha_Inicio" placeholder="SM51" value="{{ old('Fecha_Inicio') }}" required
                        class="w-full border-gray-300 rounded-md p-2 "><br>
                    @error('Fecha_Inicio')
                        <div style='color:red'>{{ $message }}</div>
                    @enderror
                ...
                <div class="w-[13%]">
                    ...
                    @csrf
                    <input type="date" name="Fecha_Final" placeholder="SM51" value="{{ old('Fecha_Final') }}" required
                        class="w-full border-gray-300 rounded-md p-2 "><br>
                    @error('Fecha_Final')
                        <div style='color:red'>{{ $message }}</div>
                    @enderror
        ...
        <form method="POST" action="{{ url('alumno') }}" class="space-y-4 ml-4">
            <div class="Datos de la empresa 2 flex gap-5">
                ...
                <div class="w-[48%] ">
                    ...
                    @csrf
                    <input type="text" name="direction_enterprise" placeholder="Introduzca la direccion de la empresa"
                        value="{{ old('direction_enterprise') }}" required
                        class=" w-full border-gray-300 rounded-md p-2 "><br>
            ...
            <div class="Datos del asesor flex gap-5">
                <div class="w-[48%] ">
                    ...
                    @csrf
                    <input type="text" name="name_advisor" placeholder="Introduzca el nombre del asesor empresarial"
                        value="{{ old('name_advisor') }}" required class=" w-full border-gray-300 rounded-md p-2 "><br>
                    @error('name_advisor')
                        <div style='color:red'>{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="Datos Personales 2 flex gap-5">
                ...
                <div class="w-[18%]">
                    ...
                    <input type="number" name="Phone_advisor" placeholder="998XXXXXXX"
                        value="{{ old('Phone_advisor') }}" required class="w-full border-gray-300 rounded-md p-2 "><br>
                    @error('Phone_advisor')
                        <div style='color:red'>{{ $message }}</div>
                    @enderror
                </div>
            ...
            <form method="POST" action="{{ url('alumno') }}" class="space-y-4 ml-4 flex">
                    ...
                    <div class="w-[98%]">
                        ...
                        @csrf
                        <textarea name="planteamiento" rows="4" placeholder="Exponen los aspectos, elementos y relaciones del problema de tu proyecto." required
                    ...
                    <div class="w-[98%]">
                        ...
                        @csrf
                        <textarea name="Justificacion" rows="4" placeholder="Escribe tu justificacion, debe manifestarse de manera clara y precisa del por qué y para qué se va llevar a cabo el estudio. Incluye causas y propósitos que motivan la 
                        investigación. Contesta las preguntas: ¿Cuáles son los beneficios que este trabajo proporcionará? ¿Quiénes serán los beneficiados? ¿Cuál es su utilidad?" required
                            class="w-full border-gray-300 rounded-md p-2">{{ old('Justificacion') }}</textarea><br>
                        @error('Justificacion')
                            <div style='color:red'>{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="w-[98%]">
                        ...
                        @csrf
                        <textarea name="activities" rows="4" placeholder="Enlista las actividades que vas a llevar a cabo de manera ordenada." required
            ...
            <div class="my-4 mr-4 gap-4 flex justify-end">
                ...
                <button class="bg-[#EEF4FB] text-primaryColor text-md font-roboto rounded-lg h-auto p-3">
                    Cerrar
                </button>
            </div>
    </div>
