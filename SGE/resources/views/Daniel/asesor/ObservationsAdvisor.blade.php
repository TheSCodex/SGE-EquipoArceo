            ...
            <div class="sm:flex w-[95%] mt-4 mx-auto pb-10">
                ...
                <div class="sm:w-full md:w-1/3 bg-primaryColor rounded-r-lg h-64 p-4 flex flex-col justify-between items-center text-center">
                    ...
                    <form action="{{ route('observations.update', $tutorComment->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <button class="bg-darkGreen border-t border-black text-white px-3 py-2 rounded-md shadow-sm hover:bg-green-dark" type="submit" onclick="return confirm('¿Estás seguro de marcar como resuelto y enviar a revisión?')">
            ...
            <div class="sm:flex flex-wrap pb-10 max-h-[80vh] overflow-auto">
                ...
                <div class="sm:w-full md:w-1/2 lg:w-1/2 xl:w-1/2 h-64 flex p-5 mb-5">
                    ...
                    <div class="w-full bg-primaryColor rounded-r-lg h-64 p-4 flex flex-col justify-between items-center text-center">
                        ...
                        @if ($comment->status != 3)
                        <form action="{{ route('observations.update', $comment->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <button class="bg-darkGreen border-t border-black text-white px-3 py-2 rounded-md shadow-sm hover:bg-green-dark" type="submit" onclick="return confirm('¿Estás seguro de marcar como resuelto y enviar a revisión?')">
                                Resolver
