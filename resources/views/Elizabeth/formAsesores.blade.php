...
<div class="w-full h-screen flex justify-center items-center bg-white">
    <form action="{{url('user')}}" method="POST" class="flex flex-col font-montserrat space-y-5 w-[500px]">
        <div class="w-full h-fit flex justify-start">
            <h1 class="text-3xl font-bold">Añadir asesor</h1>
            @csrf
        </div>
        <div class="w-full h-fit flex flex-col space-y-2">
            ...
            <div class="w-full space-y-2">
                ...
                <input type="text" name="role_user" value="{{old('role_user')}}" class="text-sm w-full rounded-md border-lightGray border-2 px-4 py-3" placeholder="Celular">
                @error('role_user')
                    <p class="text-[#ff0000] text-sm">
                        {{ $message }}
                    </p>
            ...
            <div class="w-full space-y-2">
                ...
                <input type="text" name="id_user" value="{{old('id_user')}}" class="text-sm w-full rounded-md border-lightGray border-2 px-4 py-3" placeholder="Grado academico">
                @error('id_user')
                    <p class="text-[#ff0000] text-sm">
                        {{ $message }}
                    </p>
            ...
            <div class="w-full space-y-2">
                ...
                <input type="text" name="field_user" value="{{old('field_user')}}" class="text-sm w-full rounded-md border-lightGray border-2 px-4 py-3" placeholder="Departamento">
                @error('field_user')
                    <p class="text-[#ff0000] text-sm">
                        {{ $message }}
                    </p>
        ...
        <button type="submit" class="p-2 bg-primaryColor w-[500px] rounded-md text-white">Añadir asesor</button>
    </form>
</div>
